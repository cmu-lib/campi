      ...
      <b-card header="Instructions">
        <b-card-text class="instructions">
          ...
          <ol>
            ...
            <li>Reject <BIconX />: No, this photograph doesn't belong in this set. If that photograph is a non-core photo, one that has been proposed for multiple sets, then you may see it again in a later set and have the chance to mark it as part of that later set.</li>
            <li>Exclude <BIconExclamationOctagonFill />: No, this photo doesn't belong in this set, <strong>and I want to premptorily exclude it from all other sets. It should NEVER be matched up.</strong> This is usfeul for photos you are sure will never get usefully matched (like document versos or large pictures of crowds) and will help make a lot of sets redundant.</li>
          </ol>
          <p>Once you make your selections, and choose a "primary" photo by clicking the <BIconStar /> button, click "Save" to commit those choices to the database.</p>
          <h3>Other tools</h3>
          <p>Mouse over the <BIconFolderFill /> button to see the name of the directory that this photo belongs to. Click it to open up a sidebar where you can browse all the other photos in that directory. You can do the same for the job with the <BIconCamera /> button.</p>
          ...
          <p>The <BIconSearch /> button appears on non-core photos, and will let you pull up the other match sets where this photograph may appear. (Note: it's possible that non-core photographs don't appear in other sets)</p>
